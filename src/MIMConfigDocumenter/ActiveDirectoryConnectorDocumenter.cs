...
namespace MIMConfigDocumenter
{
    ...
    internal class ActiveDirectoryConnectorDocumenter : ConnectorDocumenter
    {
        ...
        #region AD Extensions Configuration

        /// <summary>
        /// Processes the AD extensions configuration.
        /// </summary>
        protected void ProcessActiveDirectoryExtensionsConfiguration()
        {
            Logger.Instance.WriteMethodEntry();

            try
            {
                Logger.Instance.WriteInfo("Processing Active Directory Extensions Configuration.");

                this.ProcessConnectorRulesExtensionConfiguration();
                this.ProcessActiveDirectoryPasswordManagementConfiguration();
                this.ProcessConnectorPartitionDisplayNamesConfiguration();
                this.ProcessActiveDirectoryExchangeProvisioningConfiguration();
            }
            finally
            {
                Logger.Instance.WriteMethodExit();
        ...
        #region AD Password Management Configuration

        /// <summary>
        /// Processes the AD password management configuration.
        /// </summary>
        protected void ProcessActiveDirectoryPasswordManagementConfiguration()
        {
            Logger.Instance.WriteMethodEntry();

            try
            {
                Logger.Instance.WriteInfo("Processing Active Directory Password Management Configuration.");

                this.CreateSimpleOrderedSettingsDataSets(3); // 1 = Display Order Control, 2 = Property Name, 3 = Value

                this.FillActiveDirectoryPasswordManagementConfigurationDataSet(true);
                this.FillActiveDirectoryPasswordManagementConfigurationDataSet(false);

                this.CreateSimpleOrderedSettingsDiffgram();

                this.PrintActiveDirectoryPasswordManagementConfiguration();
            }
            finally
            {
                Logger.Instance.WriteMethodExit();
        ...
        #endregion AD Password Management Configuration

        #region AD Exchange Provisioning Configuration

        /// <summary>
        /// Processes the AD Exchange provisioning configuration.
        /// </summary>
        protected void ProcessActiveDirectoryExchangeProvisioningConfiguration()
        {
            Logger.Instance.WriteMethodEntry();

            try
            {
                Logger.Instance.WriteInfo("Processing Active Directory Exchange Provisioning Configuration.");

                this.CreateSimpleSettingsDataSets(2); // 1 = Property Name, 2 = Value

                this.FillActiveDirectoryExchangeProvisioningConfigurationDataSet(true);
                this.FillActiveDirectoryExchangeProvisioningConfigurationDataSet(false);

                this.CreateSimpleSettingsDiffgram();

                this.PrintActiveDirectoryExchangeProvisioningConfiguration();
            }
            finally
            {
                Logger.Instance.WriteMethodExit();
        ...
        /// Prints the connector password management configuration.
        /// </summary>
        protected void PrintActiveDirectoryExchangeProvisioningConfiguration()
        {
            Logger.Instance.WriteMethodEntry();

            try
            {
                var headerTable = Documenter.GetSimpleSettingsHeaderTable(new OrderedDictionary { { "Exchange Provisioning Setting", 40 }, { "Configuration", 60 } });
        ...
        #endregion AD Exchange Provisioning Configuration

        #endregion AD Extensions Configuration
